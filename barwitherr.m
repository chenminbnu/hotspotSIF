...
elseif isvector(values)~=isvector(errors)
    ...
    lowerErrors = zeros(size(errors,1), size(errors, 2));
    upperErrors = errors;
end
...
[nRows nCols] = size(values);
 c = linspace(0.4, 1, 4)';
 colormap([c c c])
handles.bar = bar(varargin{:},1,'LineWidth',0.3,'EdgeColor','k'); % standard implementation of bar fn
hold on
hBar = handles.bar;
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col),'.k','CapSize',3,'LineWidth',0.3);
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, 'k','LineStyle','none');
