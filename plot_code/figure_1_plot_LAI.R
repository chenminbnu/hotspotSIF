...
max_color <- 8
legend_title <- expression(LAI~'('*m^2~m^-2*')')
...
plot_hotspot_fesc <- sa_plot_other(data, expression("SZA (бу)"),expression(f[esc]),90, 2,max_color,legend_title)
data <- sif_data[c("SZA","fesc_nadir","vars")]
data$fesc_nadir <- data$fesc_nadir*pi
plot_nadir_fesc <- sa_plot_other(data, expression("SZA (бу)"),'',90, 2,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
data <- sif_data[c("SZA","fesc_hemi","vars")]
data$fesc_hemi <- data$fesc_hemi;
plot_hemi_fesc <- sa_plot_other(data, expression("SZA (бу)"),'',90, 2,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("SZA","fesc_total","vars")]
data$fesc_total <- data$fesc_total;
plot_total_fesc <- sa_plot_other(data, expression("SZA (бу)"),'',90, 2,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
...
# 36
ggsave("figure_1.tiff", plot = results.tif, width = 38, height = 25, units = "cm", dpi = 600, limitsize = FALSE, compression = "lzw")