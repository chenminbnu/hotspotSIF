...
source('function.R')
sif_data <- readMat('different_Cab_reshape.mat')
...
sif_data$vars <- sif_data$vars * 10 

sif_data <- sif_data %>%
  mutate(fesc_total = 1)

max_color <- 80
legend_title <- expression(Cab~'('*ug~cm^-2*')')## matrix to data.frame
# predefine parameter
...
plot_hotspot_luep <- sa_plot_lue(data, expression(LUE[F]),expression(LUE[P]),0.01, 0.03,max_color,legend_title)

data <- sif_data[c("sifyield","luep","vars")]
plot_sifyield_luep <- sa_plot_lue(data, expression(Phi[F]),'',0.01, 0.03,max_color,legend_title) +  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("luef_nadir","luep","vars")]
plot_nadir_luep <- sa_plot_lue(data, expression(LUE[F]),'',0.01, 0.03,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("luef_hemi","luep","vars")]
plot_hemi_luep <- sa_plot_lue(data, expression(LUE[F]),'',0.01, 0.03,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

# plot GPP

data <- sif_data[c("sif_hotspot","gpp","vars")]
plot_hotspot_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1~sr^-1*')'),expression("GPP ("*umol~m^-2~s^-1*")"),4, 33,max_color,legend_title)

data <- sif_data[c("sif_nadir","gpp","vars")]
plot_nadir_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1~sr^-1*')'),'',4, 33,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("sif_hemi","gpp","vars")]
plot_hemi_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1*')'),'',10.5, 33,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("sif_total","gpp","vars")]
plot_sifyield_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1*')'),'',25, 33,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

## plot escape ratio
data <- sif_data[c("SZA","fesc_hotspot","vars")]
data$fesc_hotspot <- data$fesc_hotspot*pi
plot_hotspot_fesc <- sa_plot_other(data, expression("SZA (║Ц)"),expression(f[esc]),90, 2,max_color,legend_title)
data <- sif_data[c("SZA","fesc_nadir","vars")]
data$fesc_nadir <- data$fesc_nadir*pi
plot_nadir_fesc <- sa_plot_other(data, expression("SZA (║Ц)"),'',90, 2,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
data <- sif_data[c("SZA","fesc_hemi","vars")]
data$fesc_hemi <- data$fesc_hemi;
plot_hemi_fesc <- sa_plot_other(data, expression("SZA (║Ц)"),'',90, 2,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
...
results.tif <- plot_grid( prow, legend_b, ncol = 2, rel_widths = c(1, .13))
...
ggsave("figure_S2_Cab.tiff", plot = results.tif, width = 38, height = 25, units = "cm", dpi = 600, limitsize = FALSE, compression = "lzw")