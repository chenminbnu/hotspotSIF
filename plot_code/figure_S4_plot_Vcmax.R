...
source('function.R')
sif_data <- readMat('different_Vcmax_reshape.mat')
...
sif_data$vars <- (sif_data$vars -1) * 30 + 10 
max_color <- 250
legend_title <- expression(Vcmax~'('*umol~m^2~s^-2*')')
...
plot_hotspot_luep <- sa_plot_lue(data, expression(LUE[F]),expression(LUE[P]),0.01, 0.04,max_color,legend_title)

data <- sif_data[c("sifyield","luep","vars")]
plot_sifyield_luep <- sa_plot_lue(data, expression(Phi[F]),'',0.01, 0.04,max_color,legend_title) +  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("luef_nadir","luep","vars")]
plot_nadir_luep <- sa_plot_lue(data, expression(LUE[F]),'',0.01, 0.04,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("luef_hemi","luep","vars")]
plot_hemi_luep <- sa_plot_lue(data, expression(LUE[F]),'',0.01, 0.04,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

# plot GPP

data <- sif_data[c("sif_hotspot","gpp","vars")]
plot_hotspot_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1~sr^-1*')'),expression("GPP ("*umol~m^-2~s^-1*")"),4, 60,max_color,legend_title)

data <- sif_data[c("sif_nadir","gpp","vars")]
plot_nadir_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1~sr^-1*')'),'',4, 60,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("sif_hemi","gpp","vars")]
plot_hemi_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1*')'),'',10.5, 60,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())

data <- sif_data[c("sif_total","gpp","vars")]
plot_sifyield_gpp <- sa_plot_other(data, expression(SIF~'('*mW~m^-2~nm^-1*')'),'',25, 60,max_color,legend_title)+  ylab(NULL) + 
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank())
...
results.tif <- plot_grid( prow, legend_b, ncol = 2, rel_widths = c(1, .2))
...
ggsave("figure_S4_vcmax.tiff", plot = results.tif, width = 38, height = 25, units = "cm", dpi = 600, limitsize = FALSE, compression = "lzw")